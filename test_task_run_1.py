import pytest

class TestMainPage():
    # номер 1
    @pytest.mark.xfail
    @pytest.mark.smoke
    def test_guest_can_login(self, browser):
        """Checks the ability to login 
        
        :param: browser: selenium browser object
    
        :return: None
        """
        assert True

    # номер 2
    @pytest.mark.regression
    def test_guest_can_add_book_from_catalog_to_basket(self, browser):
        """Checks the ability to add an item to the basket
        
        :param: browser: selenium browser object
    
        :return: None
        """
        assert True


class TestBasket():
    # номер 3
    @pytest.mark.skip(reason="not implemented yet")
    @pytest.mark.smoke
    def test_guest_can_go_to_payment_page(self, browser):
        """Checks the transition to the payment page 
        
        :param: browser: selenium browser object
    
        :return: None
        """
        
        assert True

    # номер 4
    @pytest.mark.smoke
    def test_guest_can_see_total_price(self, browser):
        """Checks the ability to see the price 
        
        :param: browser: selenium browser object
    
        :return: None
        """
        assert True


@pytest.mark.skip
class TestBookPage():
    # номер 5
    @pytest.mark.smoke
    def test_guest_can_add_book_to_basket(self, browser):
        """Checks the ability to add a book to the basket 
        
        :param: browser: selenium browser object
    
        :return: None
        """
        assert True

    # номер 6
    @pytest.mark.regression
    def test_guest_can_see_book_price(self, browser):
        """Checks the ability to see the book price 
        
        :param: browser: selenium browser object
    
        :return: None
        """
        assert True


# номер 7
@pytest.mark.beta_users
@pytest.mark.smoke
def test_guest_can_open_gadget_catalogue(browser):
        """Checks the ability to open the gadget catalogue 
        
        :param: browser: selenium browser object
    
        :return: None
        """
        assert True